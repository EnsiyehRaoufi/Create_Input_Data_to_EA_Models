"""
Convert Turtle RDF files to N-Triples, filter blank-node triples,
...
if __name__ == '__main__':
    ...
    not_blank_lines = [[],[]]
    # Process each KG file: parse Turtle, serialize to N-Triples, then collect triples
    for i in range(2):
        g = Graph()
        f = RAW_FILE_DIR+KG_FILES[i]
        g.parse(f, format=KG_FORMAT)
        fname = OUTPUT_NAMES[i]
        # Serialize RDFLib graph to N-Triples format
        ...
        with open(RAW_FILE_DIR+fname, "r") as f:
            for line in f.readlines():
                ...
                #Remove blank nodes
                #if "_:" not in line:
                # Store each triple in lowercase (blank-node filtering currently disabled)
                not_blank_lines[i].append(line.lower())
        with open(RAW_FILE_DIR+fname, 'w') as f:
            for nb in not_blank_lines[i]:
                f.write(nb+"\n")

    # Initialize new RDFLib graph for parsing reference alignment
    g = Graph()
    ...
    # Handle TTL alignment: extract owl:sameAs pairs
    if ALIGN_FORMAT == 'ttl':
    ...
    # Handle XML alignment: group entity1/entity2 URIs
    ...
    # Write cleaned same_as URI pairs to output
